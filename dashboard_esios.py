    ...
    with st.spinner('Obteniendo y procesando datos de ajuste...'):
        ...
        if dfs_precios and dfs_energia:
            try:
                ...
                if ind_d in df_p_pivot_exacto.columns and ind_rt in df_p_pivot_exacto.columns and ind_e in df_e_pivot_exacto.columns:
                    ...
                    st.markdown("### 💰 Potencial Beneficio RT Fase 2 a Bajar (Agregado Diario)")
        ...
        if df_final_energia is not None:
            ...
            for i, indicador in enumerate(df_final_energia['Indicador'].unique()):
                ...
                fig_energia.add_trace(go.Scatter(x=df_filtro[x_col], y=df_filtro['value'], name=indicador, mode='lines+markers' if perfil_24h or (freq and freq != 'h') else 'lines', line=dict(color=colores[i % len(colores)])), secondary_y=False)
            
            if spread_grafico is not None:
                fig_energia.add_trace(go.Scatter(x=df_pivot_precios.index, y=spread_grafico.values, name='Diferencia de Precio (Diario - RT2)', mode='lines+markers' if perfil_24h or (freq and freq != 'h') else 'lines', line=dict(color='rgba(150, 150, 150, 0.6)', dash='dash')), secondary_y=True)
        ...
        if df_final_secundaria is not None:
            ...
            for indicador in df_final_secundaria['Indicador'].unique():
                ...
                fig_secundaria.add_trace(go.Scatter(x=df_filtro[x_col], y=df_filtro['value'], name=indicador, mode='lines'), secondary_y="banda" in indicador.lower())
...
def pagina_intradiarios(start_date, end_date):
    ...
    st.markdown("Comparación entre el Mercado Diario, los Mercados Intradiarios de Subasta (IDA) y el Precio de Referencia MIC.")
    ...
    with st.spinner("Descargando y cruzando datos intradiarios..."):
        ...
        if diario in df_pivot.columns:
            ...
            if not spreads.empty:
                ...
                st.warning("Faltan datos de los intradiarios para calcular los spreads.")
        else:
            st.warning("No se encontró el Precio del Mercado Diario para calcular los spreads.")
